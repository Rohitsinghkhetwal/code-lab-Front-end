    ...
    <section className='sticky left-0 top-0 flex h-screen w-fit flex-col justify-between border-r border-gray-200 bg-white pt-8 text-white max-md:hidden sm:p-4 xl:p-6 2xl:w-[355px]'>
      <nav className='flex flex-col gap-4'>
        ...
        {sidebarLinks.map((items) => {
          ...
          return (
            <Link href={items.route} key={items.label} className={cn('flex gap-3 items-center py-1 md:p-3 2xl:p-4 rounded-lg justify-center xl:justify-start',{'bg-slate-700': isActive})}>
              ...
              <p className={cn('text-16 font-semibold text-black max-xl:hidden',{'!text-white': isActive})}> {items.label}</p>
            </Link>
